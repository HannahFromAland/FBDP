...
object TopMerchant {
  def main(args: Array[String]): Unit ={
    ...
    val usr_all= sc.textFile("hdfs://localhost:9000/user/hann/input/user_all").map(x => x.split(" "))
      .filter(x => x.length==9).filter(x => x(5)=="1111" && (x(7) =="1" ||  x(7) =="2" || x(7) =="3" ))
    val usr_purchase = usr_all.filter(x => x(6)=="2").map(x => (x(3),1)).reduceByKey((x,y) => x+y)
    val usr_cart = usr_all.filter(x => x(6)=="1").map(x => ((x(0),x(3)),1)).reduceByKey((x,y) => x+y)
      .map(x=> (x._1._2,1)).reduceByKey((x,y) => x+y)
    val usr_col = usr_all.filter(x => x(6)=="3").map(x => ((x(0),x(3)),1)).reduceByKey((x,y) => x+y)
      .map(x=> (x._1._2,1)).reduceByKey((x,y) => x+y)
